...
# Il frontend è il punto di ingresso utente. Espone la home del catalogo,
# chiama il backend via BACKEND_URL e propaga X-Request-Id e contesto trace.
...
def call_backend(path: str, timeout: float = BACKEND_TIMEOUT_SECONDS):
    with tracer.start_as_current_span(f"frontend.call_backend {path}") as span:
...
def proxy_products(path: str):
    ...
    return jsonify({"service": SERVICE_NAME, "version": APP_VERSION, "environment": APP_ENV, "backend_url": BACKEND_URL, "backend_status": response.status_code, "backend_response": payload, "request_id": g.request_id, **current_trace_fields()}), response.status_code
