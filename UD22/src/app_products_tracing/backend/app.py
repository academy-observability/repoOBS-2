...
app = Flask(__name__)
# Crea automaticamente uno span SERVER per ogni richiesta Flask ed estrae
# il W3C Trace Context ricevuto dal frontend.
...
def product_response(delay_seconds: float = 0.0):
    # Lo span INTERNAL rappresenta il lavoro applicativo del catalogo.
    with tracer.start_as_current_span("catalog.load_products") as span:
        span.set_attribute("app.request_id", g.request_id)
        span.set_attribute("catalog.products.count", len(PRODUCTS))
        if delay_seconds > 0:
            span.set_attribute("catalog.simulated_delay_seconds", delay_seconds)
...
def products_error():
    with tracer.start_as_current_span("catalog.load_products.error") as span:
        span.set_attribute("app.request_id", g.request_id)
        span.set_attribute("error.type", "simulated_catalog_error")
        span.set_status(Status(StatusCode.ERROR, "Simulated HTTP 500"))
        payload = {
            "service": SERVICE_NAME,
