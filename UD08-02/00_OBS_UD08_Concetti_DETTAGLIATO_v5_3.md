...
Nel laboratorio sulle risorse reali preferiamo, quando disponibile, la modalità **resource-specific**. In questa modalità Azure crea tabelle dedicate, ad esempio `StorageBlobLogs` per lo Storage Blob e `AppServiceHTTPLogs` per App Service. In configurazioni più generiche o legacy, alcuni dati possono invece confluire in `AzureDiagnostics`.
...
| Storage Blob | `StorageBlobLogs` | Operazioni di lettura, scrittura e cancellazione sui blob quando i log sono inviati in modalità resource-specific |
